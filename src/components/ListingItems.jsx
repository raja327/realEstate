    ...
    <Link className='contents' to={`/category/${listing.type}/${id}`}>
      ...
      <p className='text-[#457b9d] mt-2 font-semibold'>${listing.offer ?listing.discountedPrice .toString()
       ...
       <div className="flex items-center mt-[10px] space-x-3">
        ...
        <div className="flex items-center space-x-1">
          <p className='font-bold text-sm'>{listing.bathrooms >1 ? `${listing.bathrooms} Baths` :"1 Bath"}</p>
        </div>
       </div>
       </div>
    </Link>
    {onDelete && (<FaTrash className='absolute right-2 bottom-2 h-[14px] cursor-pointer text-red-500'
    onClick={()=>onDelete(listing.id)}/>)}

{onEdit && (<MdEdit className='absolute right-7 bottom-2 h-4 cursor-pointer'
